          ...
          for (int32_t i = 0; i < batch; ++i) { // 检查输入数据是否都符合规定的形状
            ...
            if (input_operand_shape.size() == 4) { // batch_size, channel, cols, rows
              CHECK(input_data_shape.at(0) == input_operand_shape.at(1) &&
                    ...
                    input_data_shape.at(2) ==
                        input_operand_shape.at(
                            3)); // 实际输入d的shape 应该有的 具有的shape
                                 // 输入空间大小被改变了
...
void RuntimeGraph::InitGraphParams(
  ...
  for (const auto &pair : params) {
    ...
    // 根据传入的pnnx:params
    // 进行遍历，每次遍历得到一些属性，并根据属性去初始化具体的RuntimeParameter
