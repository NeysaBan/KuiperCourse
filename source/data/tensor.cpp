...
namespace kuiper_infer
{
  ...
  void Tensor<float>::Fill(const std::vector<float> &values)
  {
    ...
    {
      auto &channel_data = this->data_.slice(i);                                                             // 按照channel取出该通道全部数据
      const arma::fmat &channel_data_t = arma::fmat(values.data() + i * planes, this->cols(), this->rows()); // 所以传入的value也应该按照channel存储
